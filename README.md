...
| `admin/seo.html` | Admin SEO Manager (guarded, LIVE) — manages `public.seo_pages` by stable page key for all nine public pages: title / meta description / keywords / canonical / robots index+follow / Open Graph / Twitter fields with character guidance (50–60 title, 140–160 description — warnings only, saves are never blocked on length), live Google-result and social-share previews, a site-media picker for share images, and honest Live / Inactive / Built-in state; public pages apply active rows (detail pages generate SEO + Product JSON-LD from the open product when no override exists) and always fall back to the tags built into their HTML |
| `admin/settings.html` | Admin Settings (guarded, LIVE) — global configuration in `public.site_settings` by stable key across six tabs (Company, Contact, Social, Website, Business features, Display): branding with logo/favicon from the Media Library, contact + WhatsApp details, social profiles, currency/language, the admin-only contact-form recipient (the database never serves it publicly), feature toggles for quotes/holds/inspections/enquiries, the announcement bar and maintenance mode; save-only-what-changed with loading state, honest validation, an Unsaved-changes chip and a leave-page warning; public pages apply saved values and keep their built-in design as the fallback |
...
- [`supabase/seo-pages.sql`](supabase/seo-pages.sql) — the `seo_pages` table (public reads active rows; active-admin writes) behind the admin SEO Manager
- [`supabase/site-settings.sql`](supabase/site-settings.sql) — the `site_settings` key/value table (public reads storefront keys, the contact-form recipient stays admin-only; active-admin writes) behind Admin Settings
