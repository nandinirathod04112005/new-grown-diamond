...
collapse toolbar, count line and UI-state stages. Customers (real
`public.profiles` rows) changes account status only through the
`admin_set_customer_status` RPC and shows live request history in its
details panel. Enquiries (10 columns, guests flagged, related products
joined from the live tables) filters by status / type / date range and
saves the status and the internal admin note through a real RLS-guarded
update with truthful toasts — no email is ever sent or faked.
