(function () {
  ...
  async function init() {
    ...
    // RLS is the authority; the explicit user filter is defence in depth and
    // also prevents accidentally broad queries if this page is changed later.
    var result = await window.ngdSupabase.from('enquiries').select('public_id,subject,message,product_type,diamond_id,jewellery_id,status,admin_note,created_at,diamonds(stock_number),jewellery(sku)').eq('user_id',auth.user.id).order('created_at',{ascending:false});
