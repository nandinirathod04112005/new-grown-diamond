...
const TARGETS = [
  ...
  'main form > div', 'main article', 'main hr',
...
/* Distance, duration and easing per kind. Phones move things less: a small
   screen exaggerates travel, and the whole block is closer to the eye. */
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          const group = node.closest('section, footer, article') || document.body;
          const index = (order.get(group) ?? 0);
          order.set(group, index + 1);
          const delay = Math.min(index * 65, 260);
