/* ============================================================
   ...
   three honest Soon items) + logout, the seven head-count KPI
...
(async () => {
  ...
  await scenario('shell: topbar profile area, 13-route sidebar (8 live), title and profile fill', {}, async (page, user) => {
    ...
    expect(state.soonCount === 3,
      'three honest Soon items — Media and Content are live management pages, got ' + state.soonCount);
    expect(['diamonds.html', 'jewellery.html', 'customers.html', 'quotes.html', 'holds.html',
      'inspections.html', 'enquiries.html', 'media.html', 'content.html'].every((h) => state.liveHrefs.includes(h)),
